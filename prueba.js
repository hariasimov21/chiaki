...
const clients = [
  ...
  */
  ...
  function listClientsIdsSortByTaxNumber() {
    // CODE HERE
    const arrClientsByrut = clients.sort((a,b) => a.taxNumber.localeCompare(b.taxNumber));
    return arrClientsByrut.map(client => client.id);
  };
  
  // 2 Arreglo con los nombres de cliente ordenados de mayor a menor por la suma TOTAL de los saldos de cada cliente en los bancos que participa.
  function sortClientsTotalBalances(cuentas = accounts, clientes = clients) {
    // CODE HERE
      let totalBalanceforClients = cuentas.reduce((contadorTotal, cadaElemento) => {
        contadorTotal[cadaElemento.clientId] = (contadorTotal[cadaElemento.clientId] || 0) + cadaElemento.balance;
        return contadorTotal
      },{});

      console.log(totalBalanceforClients);

      const totalBalanceforClientsOrdered = Object.keys(totalBalanceforClients).sort(function (a,b){
        return totalBalanceforClients[b] - totalBalanceforClients[a]
      });

      for(let x in totalBalanceforClientsOrdered){
        for(let y in clientes){
          if (clientes[y].id == totalBalanceforClientsOrdered[x]) {
            totalBalanceforClientsOrdered[x] = clientes[y].name;
        }
      }

      
    }
    return totalBalanceforClientsOrdered;
